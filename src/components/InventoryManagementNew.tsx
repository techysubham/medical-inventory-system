...
export function InventoryManagement() {
  ...
  const handleSubmitStock = async (e: React.FormEvent) => {
    ...
    try {
      const cartonData = {
        ...stockForm,
        quantityOfBoxes: parseInt(stockForm.quantityOfBoxes.toString()),
      ...
      // Create boxes
      const boxes = [];
      for (let i = 1; i <= parseInt(stockForm.quantityOfBoxes.toString()); i++) {
        boxes.push({
          boxNumber: i,
          stripsPerBox: parseInt(stockForm.stripsPerBox.toString()),
          totalStrips: parseInt(stockForm.stripsPerBox.toString()),
          availableStrips: parseInt(stockForm.stripsPerBox.toString()),
        });
      }
            ...
            <form onSubmit={handleSubmitStock} className="p-6 space-y-4">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p className="text-sm text-gray-700">
                  1 Carton → {stockForm.quantityOfBoxes} Boxes × {stockForm.stripsPerBox} Strips =
                  <span className="font-bold text-blue-600 ml-1">
                    {parseInt(stockForm.quantityOfBoxes.toString()) * parseInt(stockForm.stripsPerBox.toString())} Total Strips
                  </span>
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <input
                  ...
                  value={stockForm.cartonNumber}
                  onChange={(e) => setStockForm({ ...stockForm, cartonNumber: e.target.value })}
                  className="px-4 py-2 border border-gray-300 rounded-lg"
                  required
                />
                <input
                  ...
                  value={stockForm.quantityOfBoxes}
                  onChange={(e) => setStockForm({ ...stockForm, quantityOfBoxes: parseInt(e.target.value) || 1 })}
                  className="px-4 py-2 border border-gray-300 rounded-lg"
                  required
                />
                <input
